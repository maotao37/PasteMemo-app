...
> 状态：一期、二期均已实施（2026-09-21）。三期（{{selection}} 变量、命令面板/MCP、README）未开工。

## 实施差异（相对本设计的偏离）

- 快捷面板模板页签采用**全宽单列**（行内直接展示渲染首行），未复用「左列表+右预览」布局——模板内容短，单列信息密度更高，也避免了与 ClipItem 预览管线的耦合
- 面板置顶状态下的全局 ⌘N（`pasteDigitWhilePinned`）暂不支持模板，仅页签内 ⌘1-9 生效
- 主窗口工具栏菜单条目只加了图标与「管理模板…」入口，未加渲染摘要行（NSMenu/SwiftUI Menu 无副标题支持）
- 「管理模板…」打开设置窗口（落在上次访问的分类），未做直达模板分类的深链
