...
struct MainWindowView: View {
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 150, maximum: 220), spacing: 12)], spacing: 12) {
                ForEach(visualOrderedItems, id: \.persistentModelID) { item in
                    pinboardGridCard(item)
                        ...
                        .nativeContextMenuMonitor { mainListMenuItems(item: item) }
                        .onAppear {
    ...
    private func mainListMenuItems(item: ClipItem) -> [NativeMenuItem] {
        ...
        var groupChildren: [NativeMenuItem] = store.sidebarCounts.byGroup.filter { !$0.isSmart }.map { group in
